...
static void testParamRegistry()
{
    ...
    // Default input trim is model-reference level.
    const float def = registry.get(hexcaster::ParamId::InputGain_dB);
    CHECK(std::fabs(def) < 1e-6f, "Default InputGain_dB is not 0 dB");
